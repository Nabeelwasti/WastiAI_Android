    ...
    private fun synthesizeLocalAiResponse(prompt: String, systemInstruction: String): String {
        ...
        return when {
            ...
            lower.contains("elevenlabs") || lower.contains("azure voice") || lower.contains("online voice") || lower.contains("custom voice") -> {
                "Offline Mode: Online speech synthesis provider credentials can be configured in Wasti Settings or Account Hub."
            }
            lower.contains("groq") || lower.contains("gsk_") -> {
                "Offline Mode: Groq provider credentials can be configured in Wasti Settings for high-speed cloud reasoning."
            }
            lower.contains("connect ai") || lower.contains("openai") || lower.contains("claude") || lower.contains("deepseek") || lower.contains("gpt-4") || lower.contains("ollama") -> {
                "Offline Mode: External AI provider models can be configured in Account Hub and Settings."
            }
            lower.contains("voice") || lower.contains("female") || lower.contains("woman") || lower.contains("girl") || lower.contains("boy") || lower.contains("male") || lower.contains("آواز") -> {
                "Offline Mode: Speech synthesis persona and locale settings can be configured in Wasti Voice Settings."
            }
            lower.contains("open") || lower.contains("app") || lower.contains("camera") || lower.contains("youtube") || lower.contains("bluetooth") || lower.contains("wifi") || lower.contains("settings") -> {
                "Offline Mode: Dispatched system request for: \"${prompt.take(60)}\" to local Android intent handler."
            }
            lower.contains("how are you") || lower.contains("how r u") || lower.contains("آپ کیسے ہو") || lower.contains("کیا حال ہے") || lower.contains("کی حال اے") -> {
                "Wasti AI is operating locally. Online LLM services are currently offline or awaiting API key configuration. Local storage and offline capabilities remain available."
            }
            lower.contains("hello") || lower.contains("hi") || lower.contains("hey") || lower.contains("سلام") || lower.contains("سلا م") -> {
                "Greetings! Wasti AI OS is active. I can assist with local tasks, projects, code, and device operations."
            }
            lower.contains("urdu") || lower.contains("اردو") || lower.contains("پاکستان") || lower.contains("پنجابی") || lower.contains("punjabi") || lower.contains("kaise") || lower.contains("kya") -> {
                "سلام! میں واسطی (Wasti AI) ہوں۔ میں مقامی طور پر فعال ہوں اور آپ کی مدد کے لیے حاضر ہوں۔"
            }
            lower.contains("code") || lower.contains("function") || lower.contains("kotlin") || lower.contains("python") || lower.contains("android") -> {
                "Offline Mode: Here is a local code template for your request:\n\n```kotlin\n// Wasti Code Template\nfun handleTask(prompt: String): Boolean {\n    // Implement custom logic\n    return true\n}\n```\n\nFor real-time generative code generation, configure your Gemini API Key in Settings."
            }
            lower.contains("agent") || lower.contains("multi-agent") || lower.contains("jarvis") || lower.contains("wasti") -> {
                "Wasti OS integrates modular intelligence, local memory, and capability orchestration into a unified cognitive fabric."
            }
            lower.contains("memory") || lower.contains("remember") || lower.contains("save") || lower.contains("store") -> {
                "Offline Mode: Information has been processed by the local memory engine for indexing."
            }
            lower.contains("time") || lower.contains("date") || lower.contains("today") || lower.contains("clock") || lower.contains("وقت") || lower.contains("تاریخ") -> {
                ...
                "The current local system time is **$nowStr**."
            }
            lower.contains("news") || lower.contains("affairs") || lower.contains("weather") || lower.contains("google") || lower.contains("search") || lower.contains("خبریں") || lower.contains("موسم") -> {
                "Offline Mode: Live search grounding requires an active internet connection and configured search provider credentials."
            }
            lower.contains("project") || lower.contains("task") || lower.contains("plan") -> {
                "Offline Mode: Project planning and local workspace capabilities are available in the Projects and Code tabs."
            }
            else -> {
                "Offline Mode: I received your request: \"${prompt.take(150)}\". Please configure an API key in Settings or connect to a network for full cloud AI reasoning."
            }
        }
    }
}
