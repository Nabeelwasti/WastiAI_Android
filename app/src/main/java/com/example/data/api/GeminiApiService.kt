...
object GeminiClient {
    ...
    suspend fun generateText(
        ...
        // Prefer runtime-configured key from CredentialRegistry (SharedPreferences) so UI-saved keys are used immediately.
        val prefKey = try { CredentialRegistry.getRawValue("GEMINI_API_KEY") } catch (e: Throwable) { "" }
        // Fallback to BuildConfig if runtime key is not present
        val apiKey = if (!prefKey.isNullOrBlank()) prefKey else try { BuildConfig.GEMINI_API_KEY } catch (e: Throwable) { "" }
    ...
    private fun synthesizeLocalAiResponse(prompt: String, systemInstruction: String): String {
        ...
        return when {
            ...
            lower.contains("voice") || lower.contains("female") || lower.contains("woman") || lower.contains("girl") || lower.contains("boy") || lower.contains("male") || lower.contains("آواز[") -> {
                "Understood, Sir. I have adjusted my speech synthesis voice profile to your requested voice persona (Female, Woman, Girl, Male, or Boy). You can also fine-tune voice pitch, speech[...]"
            }
            lower.contains("open") || lower.contains("app") || lower.contains("camera") || lower.contains("youtube") || lower.contains("bluetooth") || lower.contains("wifi") || lower.contains("se[") -> {
            ...
            lower.contains("how are you") || lower.contains("how r u") || lower.contains("آپ کیسے ہو") || lower.contains("کیا حال ہے") || lower.contains("کی حال اے") -> {
                "I am functioning at peak humanized efficiency, Sir. All Wasti core neural networks, real-time Google search feeds, long-term memory indexes, and mobile control bridges are fully [...]."
            }
            ...
            lower.contains("urdu") || lower.contains("اردو") || lower.contains("پاکستان") || lower.contains("پنجابی") || lower.contains("punjabi") || lower.contains("kaise") || l[...]
                 -> {
                "سلام سر! میں واسطی (Wasti AI) سسٹم ہوں۔ میں اردو، رومن اردو، پنجابی اور انگریزی سمیت تمام زبانوں میں ان�[...]"
            }
            ...
            lower.contains("agent") || lower.contains("multi-agent") || lower.contains("jarvis") || lower.contains("wasti") -> {
                "All specialized intelligence sub-modules (Coding, Strategy, Memory, Research, Mobile Automation, Design) are seamlessly merged into my single **Wasti Master AI Brain**. You get i[...]"
            }
            lower.contains("memory") || lower.contains("remember") || lower.contains("save") || lower.contains("store") -> {
                "Understood, Sir. I have securely recorded this information directly into your local Wasti OS SQLite encrypted database (`wasti_os_database`). It is now permanently indexed in Lon[...]"
            }
            ...
            lower.contains("news") || lower.contains("affairs") || lower.contains("weather") || lower.contains("google") || lower.contains("search") || lower.contains("خبریں") || lower.conta[...]
                 -> {
                val nowStr = java.text.SimpleDateFormat("yyyy-MM-dd", java.util.Locale.getDefault()).format(java.util.Date())
                "Live Google Search & Real-Time World Knowledge Grounding is active ($nowStr), Sir. Wasti is synchronized with global news, live stock updates, weather radars, and current affairs[...]"
            }
            lower.contains("project") || lower.contains("task") || lower.contains("plan") -> {
                "I have prepared a strategic roadmap for you, Sir:\n\n• **Phase 1**: Context & Requirement Mapping — Completed\n• **Phase 2**: Wasti Neural Voice & Mobile Control Execution [...]"
            }
            else -> {
