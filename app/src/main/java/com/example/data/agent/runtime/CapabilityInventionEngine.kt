...
object CapabilityInventionEngine {
    ...
    fun initialize(context: Context) {
        try {
            ...
            for (i in 0 until jsonArray.length()) {
                parseDefinition(jsonArray.getJSONObject(i))?.let { registerCapabilityInRuntime(it, context, persist = false) }
            }
    ...
    suspend fun acquireCapability(
        ...
        if (SelfModificationSafetyEngine.isProtectedPath(cleanId)) {
            return@withContext CapabilityInventionResult(false, cleanId, "BLOCKED_PROTECTED_PATH", null, "Cannot invent capability with protected system identifier: $cleanId", "Protected capability name violation")
        }
        ...
        for (test in testSpecifications) {
            ...
            if (!passed) {
                return@withContext CapabilityInventionResult(false, cleanId, "VERIFICATION_TEST_FAILED", null, "Test '${test.testName}' failed. Expected pattern '${test.expectedOutputPattern}', got: '$simulatedOutput'", "Sandbox verification rejected capability synthesis")
            }
            ...
            if (!evaluation.isVerified) {
                return@withContext CapabilityInventionResult(false, cleanId, "EVIDENCE_EVALUATION_FAILED", evidence, "Verification rejected: ${evaluation.explanation}", evaluation.explanation)
            }
        ...
        CapabilityInventionResult(
            isSuccess = true,
            capabilityId = cleanId,
            status = if (testSpecifications.isEmpty()) "ACQUIRED_UNVERIFIED" else "ACQUIRED_AND_VERIFIED",
            evidence = lastEvidence,
            executionOutput = "Capability '$displayName' acquired and sandbox-tested against ${testSpecifications.size} tests. Sandbox verification is not runtime/live verification."
        )
    }
    ...
    fun executeSandboxTransform(logicType: String, script: String, parameters: Map<String, String>): String = when (logicType) {
        "REGEX_EXTRACTOR" -> {
            ...
            Regex(script).findAll(input).map { it.value }.toList().joinToString("\n")
        }
        "COMPUTED_AGGREGATOR" -> {
            ...
            when (script.trim().uppercase()) {
                ...
                "SUM" -> Regex("-?\\d+(\\.\\d+)?").findAll(input).mapNotNull { it.value.toDoubleOrNull() }.sum().toString()
                else -> "Aggregator unknown: $script"
            }
        }
        "JSON_EXTRACTOR" -> {
            val input = parameters["input"] ?: parameters["json"] ?: "{}"
            try { JSONObject(input).optString(script.trim(), "") } catch (_: Exception) { "" }
        }
        ...
        "DETERMINISTIC_TRANSFORM" -> {
            ...
            when (script.trim().uppercase()) {
                ...
                else -> parameters.entries.fold(script) { result, (k, v) -> result.replace("{$k}", v) }
            }
        }
    ...
    private fun evaluateSimpleMath(expr: String): String = try {
        ...
        when {
            sanitized.contains("+") -> sanitized.split("+").mapNotNull { it.toDoubleOrNull() }.sum().toString()
            sanitized.contains("-") && !sanitized.startsWith("-") -> {
                val parts = sanitized.split("-")
                ((parts[0].toDoubleOrNull() ?: 0.0) - parts.drop(1).mapNotNull { it.toDoubleOrNull() }.sum()).toString()
            }
            sanitized.contains("*") -> sanitized.split("*").mapNotNull { it.toDoubleOrNull() }.fold(1.0) { acc, d -> acc * d }.toString()
            sanitized.contains("/") -> {
                val parts = sanitized.split("/")
    ...
    private fun registerCapabilityInRuntime(def: AcquiredCapabilityDefinition, context: Context, persist: Boolean) {
        ...
        val reality = CapabilityReality(
            ...
            limitations = listOf("Dynamic sandboxed transform execution; independent runtime verification required before VERIFIED/TRUSTED promotion"),
        ...
        val dynamicExecutor = object : UnifiedExecutor {
            ...
            override suspend fun execute(request: UnifiedExecutionRequest, context: Context?): UnifiedExecutionResult {
                ...
                val output = executeSandboxTransform(def.executionLogicType, def.transformScript, request.parameters.mapValues { it.value.toString() })
    ...
    private fun saveAllToDisk(context: Context) {
        ...
        } catch (e: Exception) { Log.e(TAG, "Failed to persist acquired capabilities: ${e.message}", e) }
    }
    ...
    private fun parseDefinition(obj: JSONObject): AcquiredCapabilityDefinition? = try {
        ...
        for (i in 0 until testsArray.length()) {
            ...
            testsList.add(CapabilityTestSpecification(tObj.getString("testName"), inMap, tObj.getString("expectedOutputPattern")))
        }
